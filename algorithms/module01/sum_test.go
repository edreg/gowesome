...
func TestSum(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(fmt.Sprintf("(%v) - Sum", tc.numbers), func(t *testing.T) {
			got := Sum(tc.numbers)
			if got != tc.want {
				t.Fatalf("Sum() = %v; want %v", got, tc.want)
			}
		})
		t.Run(fmt.Sprintf("(%v) - RecursiveSum", tc.numbers), func(t *testing.T) {
			got := RecursiveSum(tc.numbers)
			if got != tc.want {
				t.Fatalf("RecursiveSum() = %v; want %v", got, tc.want)
			}
		})
	}
}
